    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if pergunta.tipo_de_perguntaid.id == 1:
                return redirect(
                    f"/Formulario/add_opcao_resposta/{pergunta.id}/%s" % formulario_id
                )
            return HttpResponseRedirect(
                "/Formulario/add_pergunta/%s?submitted=True" % formulario_id
            )
    else:
        form = pergunta_form
...
def editar_pergunta(request, pergunta_id, formulario_id):
    ...
    if p_form.is_valid():
        ...
        return HttpResponseRedirect(
            "/Formulario/editar_pergunta/%s?submitted=True" % formulario_id
        )
    return render(
        request,
