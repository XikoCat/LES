...
def remover_pergunta(request, pergunta_id, formulario_id):
    ...
    return redirect("Formulario:consultar_perguntas/{1}/%s" % formulario_id)
...
def add_opcao_resposta(request, pergunta_id, formulario_id):
    ...
    if(pergunta.temporario == True):
        state = True
    return render(request, "add_opcao_resposta.html", {'pergunta' : pergunta, 'formulario_id' : formulario_id, 'respostas' : respostas ,'form' : form, 'state' : state})
...
def editar_pergunta(request, pergunta_id, formulario_id):
    ...
    if p_form.is_valid():
        ...
        return HttpResponseRedirect('/Formulario/editar_pergunta/%s?submitted=True' % formulario_id)
    return render(request, "editar_pergunta.html", {'pergunta_edit': pergunta_edit,'formulario_id' : formulario_id, 'p_form':p_form, 'state' : state})
...
def editar_opcao_resposta(request, pergunta_id, resposta_id, formulario_id):
    ...
    return render(request, "editar_add_opcao_resposta.html", {'pergunta' : pergunta, 'formulario_id' : formulario_id, 'respostas' : respostas ,'form' : form , 'rid' : respostaid, 'state' : state })
...
def eliminar_formulario(request, formulario_id):
    ...
    formulario_pergunta = FormulárioPergunta.objects.all().filter(formulárioid = Formulario_view)
    ...
    if p_form.is_valid():
        if(pergunta_edit.temporario == False):
            ...
            new_pergunta = Pergunta(
                ...
                obrigatório = p_form.data['obrigatório'],
                tipo_de_perguntaid = get_object_or_404(TipoDePergunta, id = p_form.data['tipo_de_perguntaid']),
                temporario = True)
            ...
            for o in opções:
                new_opção = OpçãoDeResposta(
                    perguntaid = new_pergunta,
                    opção = o.opção
                ) 
                new_opção.pk = None
                new_opção.save()
            ...
            if new_pergunta.tipo_de_perguntaid.id == 1:
                return redirect(f'/Formulario/add_opcao_resposta/{new_pergunta.id}/{formulario_id}')
        else:
            p_form.save()
            if pergunta_edit.tipo_de_perguntaid.id == 1:
                return redirect(f'/Formulario/add_opcao_resposta/{pergunta_id}/{formulario_id}')
